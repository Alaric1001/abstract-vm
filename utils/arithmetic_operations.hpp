...
T add(T lhs, T rhs) {
  ...
  if (state != OperationState::Ok)
    throw get_invalid_value_exception(state);
  return lhs + rhs;
}
...
T div(T lhs, T rhs) {
  if (rhs == 0)
    throw DivisionByZero(DivisionByZero::Type::Div);
...
T mod(T lhs, T rhs) {
  if (rhs == 0)
    throw DivisionByZero(DivisionByZero::Type::Mod);
  auto state = operation_state<T>(OperationType::Div, lhs, rhs);
  if (state != OperationState::Ok)
    throw get_invalid_value_exception(state);
  return lhs % rhs;
